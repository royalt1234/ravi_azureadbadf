...
def main():
  ...
  for lib in libslist:
      ...
      print(dbfslib + ' before:' +
            getLibStatus(workspace, token, clusterid, dbfslib))
      ...
      if (getLibStatus(workspace, token, clusterid, dbfslib) != 'not found'):
          ...
          resp = requests.post(workspace + '/api/2.0/libraries/uninstall',
  ...
  if i > 0:
      ...
      resp = requests.post(workspace + '/api/2.0/clusters/restart',
                           data=json.dumps(values), auth=("token", token))
  ...
  for lib in libslist:
      ...
      resp = requests.post(workspace + '/api/2.0/libraries/install',
                           data=json.dumps(values), auth=("token", token))
      runjson = resp.text
      d = json.loads(runjson)
...
def getLibStatus(workspace, token, clusterid, dbfslib):
  resp = requests.get(
      workspace + '/api/2.0/libraries/cluster-status?cluster_id=' + clusterid, auth=("token", token))
  libjson = resp.text
